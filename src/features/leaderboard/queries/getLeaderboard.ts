...
export async function getLeaderboard() {
  ...
  // Usuarios reales
  const realLeaderboard = users.map((user, index) => ({
    rank: index + 1,
    userId: user.id,
    username: user.name,
    avatarPlayerId: user.avatarPlayerId,
  ...
  // Usuarios fake extra
  const fakeUsers = Array.from({ length: 40 }, (_, i) => ({
    rank: realLeaderboard.length + i + 1,
    userId: `fake-user-${i + 1}`,
    username: i % 5 === 0 ? `SuperUltraLongUserna${i + 1}` : `Player_${i + 1}`,
    avatarPlayerId: null,
    totalPoints: 300 - i * 4,
    rankDelta: Math.floor(Math.random() * 11) - 5,
  }));

  return [...realLeaderboard, ...fakeUsers];
}
